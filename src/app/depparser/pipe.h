...
template <
  ...
  class MaxNumberOfActionsFunction
>
class CoNLLXDependencyPipe: public fe::CommonPipeConfigure {
protected:
  /**
   * The learning mode constructor.
   *
...
public:
  ...
  bool setup() {
    ...
    if (mode == kPipeLearn) {
      ...
      _INFO << "report: loading dataset from reference file.";
      ioutils::read_conllx_dependency_dataset(ifs, dataset, forms_alphabet,
  ...
  void run() {
    ...
    for (size_t n = 0; n < N; ++ n) {
      ...
      if (mode == kPipeLearn) {
        ...
        build_output((*result.first), output);
        ioutils::write_conllx_dependency_instance((*os), output, forms_alphabet,
            lemmas_alphabet, cpostags_alphabet, postags_alphabet, feat_alphabet,
            deprels_alphabet);
      }
...
protected:
  ...
  Learner* learner;   //! The parameter learner.
  Decoder* decoder;   //! The pointer to the decoder.
  eg::TokenAlphabet forms_alphabet;     //! The alphabet for forms.
  eg::TokenAlphabet lemmas_alphabet;    //! The alphabet for lemmas.
  eg::TokenAlphabet cpostags_alphabet;  //! The alphabet for cpostags.
  eg::TokenAlphabet postags_alphabet;   //! The alphabet for postags.
  eg::TokenAlphabet feat_alphabet;      //! The alphabet for feat.
  eg::TokenAlphabet deprels_alphabet;   //! The alphabet for dependency relations.
  std::vector<CoNLLXDependency> dataset;      //! The dataset.
  std::string root;
};
