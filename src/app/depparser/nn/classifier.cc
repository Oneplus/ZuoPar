...
void NeuralNetworkClassifier::initialize(
  ...
  initialized = true;
  _INFO << "classifier: number of threads = " << nr_threads;
...
void NeuralNetworkClassifier::compute_ada_gradient_step() {
  ...
  // calculate gradient
  grad_saved.zeros();
  compute_gradient(begin, end, end- begin, cost);
...
void NeuralNetworkClassifier::take_ada_gradient_step() {
  ...
  W1 -= ada_alpha * cost.grad_W1 / arma::sqrt(eg2W1 + ada_eps);

  eg2b1 += cost.grad_b1 % cost.grad_b1;
  b1 -= ada_alpha * cost.grad_b1 / arma::sqrt(eg2b1 + ada_eps);

  eg2W2 += cost.grad_W2 % cost.grad_W2;
  W2 -= ada_alpha * cost.grad_W2 / arma::sqrt(eg2W2 + ada_eps);

  if (!fix_embeddings) {
    eg2E += cost.grad_E % cost.grad_E;
    E -= ada_alpha * cost.grad_E / arma::sqrt(eg2E + ada_eps);
  }
}

floatval_t NeuralNetworkClassifier::get_cost() { return cost.loss; }

floatval_t NeuralNetworkClassifier::get_accuracy() { return cost.accuracy; }
...
void NeuralNetworkClassifier::compute_gradient(
  ...
  arma::mat grad_W1 = arma::zeros<arma::mat>(W1.n_rows, W1.n_cols);
  arma::vec grad_b1 = arma::zeros<arma::vec>(b1.n_rows);
  arma::mat grad_W2 = arma::zeros<arma::mat>(W2.n_rows, W2.n_cols);
  arma::mat grad_E = arma::zeros<arma::mat>(E.n_rows, E.n_cols);

  floatval_t loss = 0, accuracy = 0;
