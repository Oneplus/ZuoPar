...
class Weight: public MetaFeatureParameterCollection<ScoreContext> {
public:
...
#define __REG_2(name, _W1, _W2) do {                                        \
  bfeat_map_repo.push_back(bf_map_t(                                        \
        [](const ScoreContext& ctx, std::vector<bf_t>& cache) -> void {     \
          for (const ScoreContext::T2& two: ctx.name) {                     \
...
#define __REG_3(name, _W1, _W2, _W3) do {                                   \
  tfeat_map_repo.push_back(tf_map_t(                                        \
        [](const ScoreContext& ctx, std::vector<tf_t>& cache) -> void {     \
          for (const ScoreContext::T3& three: ctx.name) {                   \
...
#define __REG_4(name, _W1, _W2, _W3, _W4) do {                              \
  qfeat_map_repo.push_back(qf_map_t(                                        \
        [](const ScoreContext& ctx, std::vector<qf_t>& cache) -> void {     \
          for (const ScoreContext::T4& four: ctx.name) {                    \
...
#define __REG_5(name, _W1, _W2, _W3, _W4, _W5) do {                         \
  q5feat_map_repo.push_back(q5f_map_t(                                      \
        [](const ScoreContext& ctx, std::vector<q5f_t>& cache) -> void {    \
          for (const ScoreContext::T5& five: ctx.name) {                    \
...
#define __REG_6(name, _W1, _W2, _W3, _W4, _W5, _W6) do {                    \
  hfeat_map_repo.push_back(hf_map_t(                                        \
        [](const ScoreContext& ctx, std::vector<hf_t>& cache) -> void {     \
          for (const ScoreContext::T6& six: ctx.name) {                     \
            cache.push_back(q5f_t(                                          \
                G(six.get<0>(), _W1),                                       \
                G(six.get<1>(), _W2),                                       \
                G(six.get<2>(), _W3),                                       \
